...
OUTPUT_ROOT="${OUTPUT_ROOT:-$WORKSPACE/swarm_test/output/secbf_runs/20260721_r35_dominant_jside_smoke2}"
...
[[ "${#RUN_DIRS[@]}" -eq 18 ]] || { echo "expected 18 runs, found ${#RUN_DIRS[@]}" >&2; exit 1; }
...
import yaml
root=Path(os.environ['OUTPUT_ROOT']); pairs=defaultdict(list)
...
print('Dominant J_side smoke audit passed: 18/18 valid and 6/6 paired hashes.')
PY
...
echo 'Dominant J_side smoke audit passed: 18/18 valid and 6/6 paired hashes.' > "$OUTPUT_ROOT/DOMINANT_JSIDE_SMOKE_AUDIT_PASS.txt"
