...
void MPC_SECBF_SOLVE::init_solver(double Ts, int N, double v_max, double v_min, double o_max,
    ...
    teacher_parameterized_ready_ = false;
    // Teacher-v1 production uses one fixed-size (max-obstacle) NLP graph.
    // Building it once avoids recompiling the CasADi/Ipopt graph for every
    // Guard candidate while leaving legacy/diagnostic modes untouched.
    if (dynamic_tau_enabled_ &&
        dynamic_tau_params_.mode == semantic_guard::DynamicTauMode::kTeacherTca &&
        cbf_metric_ == "seesm") {
        initTeacherParameterizedProblem();
    }
...
void MPC_SECBF_SOLVE::initTeacherParameterizedProblem() {
    teacher_opti_ = std::make_unique<casadi::Opti>();
    casadi::Opti& prob = *teacher_opti_;
    const int obstacle_slots = std::max(1, max_cbf_obstacles_);
    ...
    teacher_parameterized_ready_ = true;
}
...
bool MPC_SECBF_SOLVE::solve(Eigen::VectorXd* cur_state, Eigen::MatrixXd* goal_state,
    ...
    if (teacher_parameterized_ready_) {
...
bool MPC_SECBF_SOLVE::solveTeacherParameterized(
    ...
    if (obs_num > obstacle_slots || static_cast<int>(beta_list.size()) < obs_num) {
        return solveRebuilding(cur_state, goal_state, obs_matrix, beta_list);
    }
    ...
    for (int idx = 0; idx < obs_num; ++idx) {
        ...
        if (dist <= 8.0) candidates.push_back({idx, dist});
    }
    ...
    for (int obs_idx = 0; obs_idx < obs_num; ++obs_idx) {
        for (int k = 0; k < N_; ++k) {
            ...
            for (int row = 0; row < 7; ++row) obs_dm(row, obs_idx * N_ + k) = obs(row);
        }
    }
    casadi::DM beta_dm = casadi::DM::zeros(obstacle_slots, 1);
    casadi::DM cbf_mask_dm = casadi::DM::zeros(obstacle_slots, 1);
    casadi::DM side_mask_dm = casadi::DM::zeros(obstacle_slots, 1);
    for (int idx = 0; idx < obs_num; ++idx) beta_dm(idx, 0) = beta_list[idx];
    for (const int idx : selected_obstacle_indices) cbf_mask_dm(idx, 0) = 1.0;
    if (dominant_idx >= 0) side_mask_dm(dominant_idx, 0) = 1.0;
