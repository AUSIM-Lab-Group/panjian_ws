...
void maintenance_fsm(const ros::TimerEvent& e)
{
    ...
    case 2:
    {
        ...
        if(plan_success){
            ...
            // 发布搜索时间（初始规划，标记为负值表示初始规划）
            std_msgs::Float64 search_time_msg;
            search_time_msg.data = -search_time; // 负值表示初始规划
            search_time_pub_.publish(search_time_msg);
            ROS_INFO("Initial planning search time: %f seconds", search_time);
    ...
    case 4:
    {
        ...
        if(plan_success){
            ...
            // 发布搜索时间（重规划，正值）
            std_msgs::Float64 search_time_msg;
            search_time_msg.data = search_time; // 正值表示重规划
            search_time_pub_.publish(search_time_msg);
            ROS_INFO("Replan search time: %f seconds", search_time);
...
int main(int argc, char** argv){
    ...
    search_time_pub_ = nh.advertise<std_msgs::Float64>("/global_planning/search_time", 10);
